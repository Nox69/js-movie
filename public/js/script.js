      ...
      <div class="container">
       <div class ="imgfit">
         <div class = "card">
           ...
           <div><h6>${movie.releaseDate}</h6></div>`;
               if (isFav) {
                     movieWrapperLst = movieWrapperLst + `</div></div></div></li>`;
               } else {
                     movieWrapperLst = movieWrapperLst + `<button class="btn btn-primary"
                     onclick="addFavourite(${movie.id})">
                     Add to favourites</button></div>
                     </div></div></li>`;
  }
   return movieWrapperLst;
 };
 }
...
function addFavourite(id) {
  ...
  //  alert("i guess you like this movie alot, its already your favourite")
     ...
     //document.write("<h1>Already favourited the movie</h1>");
